...
function destinationExists(destinationFile) {
...
function getFormattedName(name, cmd, type, appendType) {
    if (type === void 0) { type = ""; }
    if (appendType === void 0) { appendType = false; }
    var lowerCased = ["middleware"];
    ...
    if (cmd.plural) {
        ...
        if (lowerCased.includes(type))
            names[0] = lodash_1.default.lowerCase(names[0]);
        ...
        if (lowerCased.includes(type))
            names[0] = lodash_1.default.lowerCase(names[0]);
        names[names.length - 1] = pluralize_1.default.singular(lastName);
    }
    if (appendType) {
